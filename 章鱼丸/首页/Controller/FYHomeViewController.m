...
@interface FYHomeViewController()<UITableViewDataSource,UITableViewDelegate,FY31Block2Delegate,FYFiveViewCellDelegate,FY33Block3Delegate,FYSevenDelegate,FYEbannerViewCellDelegate,FYMbannerViewCellDelegate,FYMeiDelegate,UISearchBarDelegate>
...
@property (nonatomic, strong) FYHomepageModel *homepageM;

@property (nonatomic, strong) NSMutableArray *bannersArray;
@property (nonatomic, strong) NSMutableArray *categoryArray;
@property (nonatomic, strong) NSMutableArray *recommendArray;
@property (nonatomic, strong) FYHomespecialModel *specialModel;
@property (nonatomic, strong) FYHometoptenModel *topenModel;
...
-(void)initTableview
{
    ...
    self.tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height-70) style:UITableViewStyleGrouped];
...
{
    if (_homepageM && _homeGroupM )//没加载到数据就不显示了
    {
        return 9;
    }else{
        return 0;
        ...
        else if (indexPath.section == 7)
        {
            ...
            if (_homeGroupM.hotService)
            {
                ...
                }else if(indexPath.row == 2)
                {
                    ...
                    if(self.cell7 == nil)
                    {
                        self.cell7 = [[FYFiveViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIndentifier Array:self.bannersArray];
                    }
...
-(void)didSelectedFiveViewCellIndex:(NSInteger)index
{
    ...
    FYHomebannersModel *banner = _homepageM.banners[index];

    NSString *cont = banner.cont;
                                                     ...
                                                     completionHandler: ^(NSData *data, NSURLResponse *response, NSError *error)
                                      {
                                          if (error) {
                                              ...
                                              NSDictionary *jsonObject = [NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
                                               ...
                                               _homepageM=[MTLJSONAdapter modelOfClass:[FYHomepageModel class] fromJSONDictionary:jsonObject[@"data"] error:nil];
                                              
                                              _bannersArray = [[NSMutableArray alloc] initWithArray:_homepageM.banners];//横幅
                                              _categoryArray = [[NSMutableArray alloc] initWithArray:_homepageM.category];//最上滑窗
                                              _recommendArray = [[NSMutableArray alloc] initWithArray:_homepageM.recommend];//空格
                                              
                                              _specialModel = _homepageM.special;//活动
                                              _topenModel = _homepageM.topten;//抢购
                                              
                                              //NSLog(@"抓抓抓 - - %@",jsonObject[@"data"]);
